...
class GridClusteringApp:
    ...
    def _build_left_panel(self, parent):
        ...
        info = _section("Thông tin Sinh viên")
        ...
        data_sec = _section("① Dữ liệu")
        tk.Label(data_sec, text="Số điểm dữ liệu:", bg="#2C3E50", fg="#BDC3C7").pack(anchor=tk.W)
        self.n_samples_var = tk.IntVar(value=400)
        _entry(data_sec, self.n_samples_var)

        tk.Label(data_sec, text="Số cụm thực tế:", bg="#2C3E50", fg="#BDC3C7").pack(anchor=tk.W)
        self.n_centers_var = tk.IntVar(value=3)
        _entry(data_sec, self.n_centers_var)

        _btn(data_sec, "🔄 Tạo ngẫu nhiên", self.cmd_generate_data, "#27AE60")
        _btn(data_sec, "📂 Tải file CSV", self.cmd_load_csv, "#8E44AD")

        param_sec = _section("② Tham số CLIQUE")
        tk.Label(param_sec, text="Số ô chia mỗi trục (k):", bg="#2C3E50", fg="#BDC3C7").pack(anchor=tk.W)
        self.k_var = tk.IntVar(value=10)
        _entry(param_sec, self.k_var)

        tk.Label(param_sec, text="Ngưỡng mật độ (MinPts ξ):", bg="#2C3E50", fg="#BDC3C7").pack(anchor=tk.W)
        self.xi_var = tk.IntVar(value=6)
        _entry(param_sec, self.xi_var)

        ctrl_sec = _section("③ Điều khiển")
        _btn(ctrl_sec, "🔍 Xem Lưới trung gian", self.cmd_show_grid, "#E67E22")
        _btn(ctrl_sec, "▶ Chạy Phân Cụm", self.cmd_run_algo, "#E74C3C")
        _btn(ctrl_sec, "💾 Xuất kết quả", self.cmd_export, "#7F8C8D")

        stat_sec = _section("④ Trạng thái")
    ...
    def cmd_generate_data(self):
        try:
            self.X = generate_mock_data(self.n_samples_var.get(), self.n_centers_var.get(), 1.2)
    ...
    def cmd_show_grid(self):
        ...
        k, xi = self.k_var.get(), self.xi_var.get()
